...
class Database {
    ...
    public newUser(user: User): string {
...
export class User {
    ...
    constructor(oAuthTokenResponse: OAuthTokenResponse) {
        ...
        this.access_token = oAuthTokenResponse.access_token;
        this.refresh_token = oAuthTokenResponse.refresh_token;
        this.expires_at = new Date(
            Date.now() + oAuthTokenResponse.expires_in * 1000,
        );
        this.token_type = oAuthTokenResponse.token_type;
    }

    public refreshedAccessToken(oAuthTokenResponse: OAuthTokenResponse): void {
        this.access_token = oAuthTokenResponse.access_token;
        this.refresh_token = oAuthTokenResponse.refresh_token;
        this.expires_at = new Date(
            Date.now() + oAuthTokenResponse.expires_in * 1000,
...
export async function onOAuthCallback(
    ...
    const user = new User(response.oAuthTokenResponse);
    const userToken = database.newUser(user);
...
export async function refreshAccessTokenIfNeeded(
    ...
    user.refreshedAccessToken(response.oAuthTokenResponse);
